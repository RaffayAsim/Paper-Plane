...
async function run() {
  ...
  console.log('\n=== IMPORTING LEAD WITH METADATA ===');
  const todayStr = new Date().toISOString().split('T')[0];
  console.log(`Today's date is: ${todayStr}`);

  const importPayload = {
    campaignName: 'Automated Event Test Scrape',
    leads: [
      {
        name: 'Automated Test Lead',
        email: 'noreply@bookmarket.ai',
  ...
  const campaignPayload = {
    ...
    bodyTemplate: 'Hi {{name}},\n\nThis is an automated event reminder. Your contract renewal date is {{contractRenewalDate}}.\n\nBest,\nEmailPulse Team',
    triggerRule: {
